...
The starting `densfit-sigma` value is 0.6 nm, which essentially turns every structure into a shapeless blob - this is what we want during the early stage. The final `densfit-sigma` value should correspond to the maximum available from the croy-EM map. Now, it's hard to say what this maximum is. One way to estimate this value is to take a structure that fits the map sufficiently well and cross-correlate both in real-space at different simulated map resolutions (using *e.g.*, UCSF Chimera or SPIDER). Typical sigma values would be around 0.2-0.3 nm. Going below 0.2 nm, however, should be avoided because "sharp" similated maps can produce high pulling forces and hence LINCS instabilities.
